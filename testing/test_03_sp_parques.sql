...
-- Test 5: alta con CostoHectarea <= 0.
-- Resultado esperado: error del motor por violar CK_Parque_CostoHectarea
BEGIN TRY
    DECLARE @idParqueTest5 INT;
    EXEC USP_AltaParque
        @Nombre = 'Parque Nacional Test CostoHectarea Invalido',
        @HorarioCierre = '18:00',
        @HorarioApertura = '08:00',
        @Superficie = 500.00,
        @CostoHectarea = 0.00,
        ...
        @Localidad = 'Puerto Iguazu',
        @TipoParque = 'Parque Nacional',
        @IdParque = @idParqueTest5 OUTPUT;
    PRINT 'Test 5 - FALLO: debería haber lanzado un error por CK_Parque_CostoHectarea.';
END TRY
BEGIN CATCH
...
-- Test 6: modificación parcial (se modifica HorarioCierre).
...
SELECT Test = 6, * FROM Parques.Parque WHERE IdParque = @idParqueTest1;

-- Test 7: modificación de un IdParque inexistente.
-- Resultado esperado: no se puede modificar porque el parque indicado no existe
BEGIN TRY
    EXEC USP_ModificacionParque
        @IdParque = -1,
        @Nombre = 'No debería aplicarse';
    PRINT 'Test 7 - FALLO: debería haber lanzado un error por parque inexistente.';
END TRY
BEGIN CATCH
    PRINT 'Test 7 - OK. Error esperado capturado: ' + ERROR_MESSAGE();
END CATCH

-- Setup para Test 8/9: un segundo parque para probar el cruce de nombres en la modificación.
DECLARE @idParqueTest8 INT;
EXEC USP_AltaParque
    ...
    @IdParque = @idParqueTest8 OUTPUT;

-- Test 8: modificación que intenta renombrar el parque del Test 8 con el nombre que ya usa el parque del Test 1.
-- Resultado esperado: no se puede modificar el parque porque ya existe otro parque con ese nombre
BEGIN TRY
    EXEC USP_ModificacionParque
        @IdParque = @idParqueTest8,
        @Nombre = 'Parque Nacional Test Iguazu';
    PRINT 'Test 8 - FALLO: debería haber lanzado un error por nombre duplicado.';
END TRY
BEGIN CATCH
    PRINT 'Test 8 - OK. Error esperado capturado: ' + ERROR_MESSAGE();
END CATCH

-- Test 9: modificación que reenvía el mismo nombre que ya tiene el propio parque
...
SELECT Test = 9, * FROM Parques.Parque WHERE IdParque = @idParqueTest8;

----------------------------------------
-- USP_BajaParque
----------------------------------------

-- Test 10: baja de un IdParque inexistente.
-- Resultado esperado: no se da ninguna baja porque el parque indicado no existe
BEGIN TRY
    EXEC USP_BajaParque @IdParque = -1;
    PRINT 'Test 10 - FALLO: debería haber lanzado un error por parque inexistente.';
END TRY
BEGIN CATCH
...
EXEC USP_BajaParque @IdParque = @idParqueTest8;

SELECT * FROM Parques.Parque WHERE IdParque = @idParqueTest8;
...
-- Test 12: baja de un parque con registros relacionados (Actividad).
...
BEGIN TRY
    ...
    PRINT 'Test 12 - FALLO: debería haber lanzado un error por FK relacionada.';
END TRY
BEGIN CATCH
...
-- Cierre
----------------------------------------
-- Se descartan todos los datos de prueba (parques y la actividad del Test 12)
